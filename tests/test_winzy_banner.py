...
def test_display_banner(mock_ctk, mock_frame, mock_label, mock_button):
    ...
    mock_window.attributes.side_effect = lambda *args: 0 if args[0] == '-alpha' else None
    ...
    font_size = max(min(window_width // (len("Hello World") // 2 + 1), window_height // 2), 20)
    mock_label.assert_called_once_with(
        mock_frame.return_value, text="Hello World", font=("Helvetica", font_size, "bold"), text_color="white"
    )
    mock_label.return_value.pack.assert_called_once_with(expand=True, fill="both", padx=20, pady=20)
